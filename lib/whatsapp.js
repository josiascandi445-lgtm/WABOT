...
async function startWhatsApp(onMessage) {
  ...
  if (!state.creds.registered) {
    ...
    try {
      console.log("PAIRING_NUMBER:", pairingNumber);

      // Espera alguns segundos para estabilizar o socket
  ...
  sock.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(
        `🔌 Conexão fechada (code: ${statusCode})`
      );
      ...
      if (reconnectAttempts > MAX_RECONNECT) {
        console.log(
          "❌ Muitas tentativas de reconnect. Encerrando."
        );
        process.exit(1);
      }
...
module.exports = { startWhatsApp };      console.log(`🔄 Reconectando... (${reconnectAttempts}/${MAX_RECONNECT})`);
